...
const login = (async(req,res)=>{
    await userSignupData.findOne({Email:req.body.userData.email}).then(async(result)=>{
        if(result && result.Flag === false){
            const validpassword = await bcrypt.compare(req.body.userData.password,result.Password)
            if(!validpassword){
...
const googleLogin = (async(req,res)=>{
    userSignupData.findOne({Email : req.user.userData.email}).then(async(data1)=>{
        if(data1){
            if(data1.flag===false){
                res.status(200).json({'res':'User Account Already Exist.Try with other E mail-id'})
            }
            else{
                detailsArray[0]=data1.FirstName.toUpperCase();
                detailsArray[1]=data1.LastName.toUpperCase();
                detailsArray[2]=data1.Email;
                res.status(200).json({data:data1,value:data1.profileimage.data.toString('base64')})
            }
        }
        else{
            axios
